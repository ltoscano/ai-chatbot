    ...
    case 'object': {
      if (jsonSchema.properties) {
        ...
        if (Object.keys(shape).length === 0) {
          console.warn(
            'Empty object schema detected, adding default any field for OpenAI compatibility',
          );
  ...
  try {
    ...
    for (const mcpTool of response.tools) {
      ...
      try {
        ...
        console.error(
          `❌ Failed to process MCP tool ${mcpTool.name}:`,
          schemaError,
        );
        console.error(
          `📋 Problematic schema:`,
        ...
        // Crea un tool fallback che restituisce un errore informativo
        dynamicTools[toolName] = tool({
          description: `[MCP - Schema Error] ${mcpTool.description || `Tool from MCP: ${mcpTool.name}`}`,
          parameters: z.object({}), // Schema vuoto come fallback
          execute: async () => {
            return {
              success: false,
              error: 'Schema validation error',
              message: `Il tool MCP ${mcpTool.name} ha uno schema non compatibile con OpenAI. Errore: ${schemaError instanceof Error ? schemaError.message : 'Unknown error'}`,
            };
          },
        });

        console.log(`⚠️ Registered fallback tool for: ${toolName}`);
      }
    }
